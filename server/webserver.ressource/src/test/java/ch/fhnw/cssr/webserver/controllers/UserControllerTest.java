...
public class UserControllerTest {
    ...
    public void resetPassword() throws Exception {
        ...
        User existing = userRepository.findByEmail("hansli@irgendwas.ch");
        ResetPasswordParameters prms = new ResetPasswordParameters();
        prms.setOldPasswordTempToken(true);
        prms.setOldPassword(existing.getTempToken());
        
        String anicePwd = "helloWorld_{!p249zt982öä.efdh8wa";
        prms.setNewPassword(anicePwd);
        
        mockMvc
        .perform(put("/user/me/password")
                .header("Accept", "application/json")
                .contentType(MediaType.APPLICATION_JSON)
                .content(TestUtils.json(prms)))
                .andExpect(status().is2xxSuccessful());
    }
